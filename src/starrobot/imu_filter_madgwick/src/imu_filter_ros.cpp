  ...
  initialized_(false)
{
  ...
  if (!nh_private_.getParam ("angular_scale", angular_scale_))
    angular_scale_ = 1.0;

  ROS_INFO("angular_scale : %f", angular_scale_);
...
void ImuFilterRos::publishFilteredMsg(const ImuMsg::ConstPtr& imu_msg_raw)
{
  ...
  imu_msg->angular_velocity.z = imu_msg_raw->angular_velocity.z * angular_scale_;
